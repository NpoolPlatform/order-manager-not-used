  ...
  stages {
    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists order_manager;"
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep order-manager | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            TAG=$feature_name DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep order-manager | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep order-manager | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            tag=`git describe --tags $revlist`
            set +e
            docker images | grep order-manager | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/order-manager:latest/order-manager:$feature_name/g" cmd/order-manager/k8s/02-order-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/order-manager/k8s/02-order-manager.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/order-manager/k8s/02-order-manager.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/order-manager:latest/order-manager:$tag/g" cmd/order-manager/k8s/02-order-manager.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/order-manager/k8s/02-order-manager.yaml
